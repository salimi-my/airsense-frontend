/*
 * CUSTOMIZATIONS MADE TO MULTI SELECT COMPONENT:
 *
 * 1. Added asChild prop to the PopoverTrigger component
 * 2. Added border-input and aria-invalid styles to the trigger button to match Input appearance
 * 3. Fixed Close/Clear button centering by hiding the trailing invisible CheckIcon
 * 4. Fixed vertical separator between Clear and Close using self-stretch
 * 5. Updated checkbox style to match checkbox.tsx (rounded-[4px], border-input
 *    when unchecked, border-primary bg-primary when checked, shadow-xs, size-3.5 icon)
 * 6. Removed margin from the Remove button to align with the checkbox
 *
 * This is a custom component built on top of shadcn/ui's components.
 */
...
const multiSelectVariants = cva("m-1 transition-all duration-300 ease-in-out", {
  variants: {
    variant: {
      default: "border-input text-foreground bg-accent/60",
      secondary:
...
interface MultiSelectProps
  ...
  /**
   * Infinite loading support. When true, allows requesting next page while scrolling.
   */
  hasMore?: boolean;
  ...
  /**
   * Called when options list reaches bottom.
   */
  onReachEnd?: () => void;
}
        ...
        >
          ...
          <PopoverTrigger asChild>
            <Button
              ...
              className={cn(
                "border-input aria-invalid:border-destructive aria-invalid:ring-destructive/20 dark:aria-invalid:border-destructive/50 dark:aria-invalid:ring-destructive/40 flex h-auto min-h-10 items-center justify-between rounded-md border bg-inherit p-1 hover:cursor-text hover:bg-inherit aria-invalid:ring-3 [&_svg]:pointer-events-auto",
                        ...
                        return (
                          ...
                          >
                            ...
                            <div
                              ...
                              className="-m-0.5 ml-0 h-4 w-4 cursor-pointer rounded-sm p-0.5 hover:bg-white/20 focus:ring-1 focus:ring-white/50 focus:outline-none"
                            >
                              <XCircle
                                className={cn(
                                  "h-3! w-3!",
                ...
                {!hideSelectAll && !searchValue && (
                  <CommandGroup>
                    ...
                    >
                      <div
                        className={cn(
                          "mr-2 flex h-4 w-4 shrink-0 items-center justify-center rounded-[4px] border shadow-xs transition-shadow",
                          selectedValues.length ===
                    ...
                    {selectedValues.length > 0 && (
                      <>
                        <CommandItem
                          ...
                          className="flex-1 cursor-pointer justify-center [&>svg:last-child]:hidden"
                        >
                          Clear
                        </CommandItem>
                        <Separator
                          orientation="vertical"
                          className="min-h-6 self-stretch"
                        />
                      </>
                    )}
                    <CommandItem
                      onSelect={() => setIsPopoverOpen(false)}
                      className="max-w-full flex-1 cursor-pointer justify-center [&>svg:last-child]:hidden"
                    >
                      Close
                    </CommandItem>
                  </div>
